...
async def insert_to_db(connection: Connection, document: Dict, embed_model: ModelManager) -> Tuple[Dict, Dict]:
    # Tokens usage will need to be calculated based on your model's specifics
    tokens_usage = {"prompt_tokens": 0, "total_tokens": 0}

    # Use the model to generate embeddings for the document text
    embedding, tokens = await embed_model.embed_text(document['text'])
    tokens_usage['prompt_tokens'] += tokens['prompt_tokens']  # Adjust based on actual tokens used
    tokens_usage['total_tokens'] += tokens['total_tokens']    # Adjust based on actual tokens used

    # Start transaction to insert collection, sections, and embeddings
    async with connection.transaction():
        ...
        collection_id = await connection.fetchval(
            ...
            ''', 
            document['document_title']  # Assuming the document title is the collection name
        )
        ...
        for section in document['structure']:
            ...
            section_embedding, _ = await embed_model.embed_text(section['text'])
