...
const { AnalystUser } = require("./workspaceUsers");
...
const Analyst = {
  ...
  new: async function (name = null, creatorId = null) {
    ...
    try {
      ...
      if (!!creatorId) await AnalystUser.create(creatorId, workspace.id);
  ...
  getWithUser: async function (user = null, clause = {}) {
    ...
    try {
      ...
      return {
        ...workspace,
        documents: await Document.forAnalyst(workspace.id),
      };
    } catch (error) {
      console.error(error.message);
      return null;
  ...
  whereWithUsers: async function (clause = {}, limit = null, orderBy = null) {
    try {
      ...
      for (const workspace of workspaces) {
        const userIds = (
          await AnalystUser.where({ workspace_id: Number(workspace.id) })
        ).map((rel) => rel.user_id);
  ...
  updateUsers: async function (workspaceId, userIds = []) {
    try {
      await AnalystUser.delete({ workspace_id: Number(workspaceId) });
      await AnalystUser.createManyUsers(userIds, workspaceId);
